# ADR 0001: @plasius/game-audio-react Package Boundary
...
Keep React and Player System integration in a separate package so browser playback and core command contracts can be consumed outside React applications.

This package uses `game.audio.react-provider.enabled` as its initial rollout flag. Product hosts remain responsible for remote flag evaluation and any capability checks required for user-visible controls or diagnostics.
