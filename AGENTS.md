...
- Use `/Users/philliphounslow/plasius/schema` (`@plasius/game-audio-react`) as the baseline template when creating new `@plasius/*` packages.
...
### Local Notes
This repo is `@plasius/game-audio-react`, react provider and hook contract scaffold for listener pose, emitter registration, preferences, captions, and Player System cues.
